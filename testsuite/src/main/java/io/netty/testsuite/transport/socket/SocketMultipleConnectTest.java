...
public class SocketMultipleConnectTest extends AbstractSocketTest {
    ...
    public void testConnectPending(ServerBootstrap sb, Bootstrap cb) throws Throwable {
        Channel sc = null;
        Channel cc = null;
        try {
            // No AUTO_READ as we want to ensure the first connect is still pending when issue the second one.
            ...
            ChannelFuture connectFuture = cc.connect(sc.localAddress());
            ChannelFuture connectFuture2 = cc.connect(sc.localAddress());
            sc.read();
            assertTrue(connectFuture.awaitUninterruptibly().isSuccess());
            assertTrue(connectFuture2.awaitUninterruptibly().cause() instanceof ConnectionPendingException);
        } finally {
            if (cc != null) {
                cc.close();
            }
